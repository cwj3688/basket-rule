...
export default function Home() {
  ...
  const handleSearch = (query: string) => {
    ...
    if (query.trim()) {
      router.push(`/${language}/rules?q=${encodeURIComponent(query)}`);
    }
  };

  const quickAccessItems = [
    { href: `/${language}/rules`, icon: BookOpen, title: dict.nav.rules, desc: dict.home.qa_rules_desc, color: "text-blue-500", bg: "bg-blue-500/10" },
    { href: `/${language}/quiz`, icon: Trophy, title: dict.nav.quiz, desc: dict.quiz_page.title, color: "text-yellow-500", bg: "bg-yellow-500/10" },
    { href: `/${language}/community`, icon: Gavel, title: dict.nav.community, desc: dict.community_page.title, color: "text-rose-500", bg: "bg-rose-500/10" },
    { href: `/${language}/compare`, icon: Scale, title: dict.nav.compare, desc: dict.home.qa_compare_desc, color: "text-orange-500", bg: "bg-orange-500/10" },
    { href: `/${language}/signals`, icon: HandMetal, title: dict.nav.signals, desc: dict.home.qa_signals_desc, color: "text-green-500", bg: "bg-green-500/10" },
    { href: `/${language}/local-rules`, icon: MapPin, title: dict.nav.local, desc: dict.home.qa_local_desc, color: "text-purple-500", bg: "bg-purple-500/10" },
  ];
